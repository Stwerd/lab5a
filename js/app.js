...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  let add = sum(a, b).shift();
  let add2 = sum(add, c).shift();
  let times = multiply(a, b).shift();
  let times2 = multiply(times, c).shift();
  let addMess = `${a} and ${b} and ${c} sum to ${add2}.`;
  let multiplyMess = `The product of ${a} and ${b} and ${c} is ${times2}.`;
  let arr = [add2, times2, addMess, multiplyMess];
  return arr;
}
...
function multiplyAnyArray(dynamicArray) { //eslint-disable-line
  ...
  for (let i = 0; i < dynamicArray.length; i++) {
    arrayTimes = multiply(arrayTimes, dynamicArray[i])[0];
  }
  let message = ` have a product of ${arrayTimes}.`;
  let numbers = '';
  for (let z = 0; z < dynamicArray.length; z++) {
    if (z === dynamicArray.length - 1) {
      numbers += `${dynamicArray[z]}`;
    }
    else if (z !== dynamicArray.length) {
      numbers += `${dynamicArray[z]},`;
    }
  }
  message = 'The numbers ' + numbers + message;
